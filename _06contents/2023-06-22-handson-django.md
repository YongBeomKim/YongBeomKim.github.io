...
그리고 개정판에 추가되었으면 하는 내용은 챕터들을 마무리 할때, **Mini Project** 형식으로  문제를 출제함으로써 독자가 스스로 해결해 나아가는 경험이 있었으면 좋겠습니다. 답은 Github 등에 코드를 공개함으로써, 실무경험이 부족한 독자들에게 미니 프로젝트를 경험할 수 있는 내용들이 추가되었으면 좋을거 같습니다.

각 챕터를 통해 이해한 내용들을 마무리 연습문제로 제시하고, 각각의 연습문제 결과가 모여서 **이메일 입력한 독자들에게 날씨를 아침마다 알려주는 메일링 서비스를 제공하는 API 서버** 처럼  하나의 완성된 프로젝트가 되는 형태로 추가되면 좋을거 같습니다.

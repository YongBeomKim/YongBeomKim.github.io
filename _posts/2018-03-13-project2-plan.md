...
2 **Collaborative Filter** : 평점행렬의 패턴을 활용하여 예측 
3 **Neighborhood 모형** : 유사한 사용자에게 가중치를 부여(user based)
4 **유사도 예측** : 평균제곱, sin/cos, 피어슨 유사도를 활용 
3 **KNN** 가중치 예측 : 유사도 벡터공간에서 가까운 k명의 사용자 값을 가중평균 하여 예측
6 **Matrix Factorization** : 오차함수를 최소로하는 요인벡터를 찾는다
7 **Singular Value Decomposition** : 특이값 분해

...
## v-show : 실시간으로 Render() 되어 DOM에 적용

위에서 message 객체를 사용하는 대신, **Vue 인스턴스**의 **$data** 파라미터를 직접 활용 가능하다 (**Json 객체를 출력한다**)
...
> v-if = "!message" : message 에 값이 없을 때 실행
...
<script>
  new Vue({
    el: '#app',
    data: { message: 'Our king is dead! Send help!' }
  })
</script>
```

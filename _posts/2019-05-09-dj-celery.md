...
Django 에서 비동기 처리 방식으로, **Channels** 와 **Celery** 가 있습니다. **[Channels](https://github.com/django/channels)** 는 **비동기 방식(A sync)** 으로 챗봇등에 활용되고, **[Celery](https://github.com/celery/django-celery)** 는 **Scheduler** 로써 활용 됩니다.
...
```
**[Celery 공식문서](http://docs.celeryproject.org/en/master/django/first-steps-with-django.html)** 에 따르면 별도의 **[Django-celery](https://github.com/celery/django-celery)** 등의 모듈이 없어도 **Celery** 를 Django 에서 사용하실 수 있습니다.
{: .notice--info}
...
--- * ***  * -- Linux-4.13.0-46-generic-x86_64-Ubuntu
-- * - **** --- [config]
- ** ---------- .> app:         server:0x7f79ffd22ac8
- ** ---------- .> transport:   redis://localhost:6379//
- ** ---------- .> results:     redis://localhost:6379/
- *** --- * --- .> concurrency: 4 (prefork)
-- ******* ---- .> task events: OFF (enable -E monitor worker)
```
...
## App/tasks.py
`app.autodiscover_tasks()` 에서 정의한 내용으로, App 폴더내 `tasks.py` 의 내용을 바탕으로 스케줄링을 하겠습니다. 주의할 점은 `mysite/tasks.py` 에서는 작동이 안되고 꼭 개별 **App** 의 폴더에서 작업들을 정의해야 합니다.
...
예제 내용을 확인하다 보면 `@` 데코레이터를 여러가지로 설명이 되고 있지만, 정식 문서에서 정의한 대로 `from celery import shared_task` 으로 일원화 하여 활용 하도록 합니다.
{: .notice--info}

## run the App/tasks.py

> **$ celery -A mysite worker -l info**

> **$ celery -A mysite worker -l DEBUG -E**

모니터링을 위해 `Redis` 서버를 실행합니다.

> **$ python manage.py shell**

Django Shell 을 실행하여 테스트를 진행합니다.

```python
In [1]: from stock.tasks import mul
In [2]: mul.delay(10,2)
Out[2]: <AsyncResult: 2e0c8bd8-15e4-496f-a73b-05043f9c43>

In [3]: mul.apply_async(10,2)
Out[3]: <AsyncResult: 5e3e4fe3-35e4-645f-b35b-521435k9e88>
```

**.delay(arg, kwarg=value)** 또는 **apply_async(eta=now + timedelta(seconds=10))** 메소드를 사용하면 비동기로 실행이 되고, 입력에 따른 Redis 반응 결과값을 관찰할 수 있습니다. **[calling문서](http://docs.celeryproject.org/en/latest/userguide/calling.html)**

```r
[2019-05-30 MainProcess] Received task: app.tasks.mul[2e0c8]  
[2019-05-30 INFO/ForkPoolWorker-2] Task app.tasks.mul[2e0c] 
succeeded in 0.0037999300002411474s: 100
```
...
## supervisor
redis 에서 처리되는 프로세서 내용을 **daemon** 으로 위탁 관리하는 도구 입니다.
...
```
설정 파일들은 `/etc/supervisor/conf.d/` 에서, 로그파일은 `/var/log/supervisor
` 에서 관리 합니다.

작업파일과 연결을 위해 다음의 설정 파일을 생성한 뒤 **[다음의 내용](https://medium.com/@yehandjoe/celery-4-periodic-task-in-django-9f6b5a8c21c7)** 내용을 추가 합니다. 설정값을 입력할 때에는 다음의 3가지를 설정에 맞춰서 입력 합니다.

1. **Celery** program 경로 (virtualenv 사용)
2. the **Django project** 경로
3. **user** 이름

> **$ sudo nvim /etc/supervisor/conf.d/celery_mysite_worker.conf**
...
; true, start automatically supervisord restarted
autostart=true
...
> **$ sudo nvim /etc/supervisor/conf.d/celery_mysite_beat.conf** 
...
**[django-celery-result](https://github.com/celery/django-celery-results)** 는 Celery 의 Schedule 을 Django 의 ORM 에 저장하여 안정적으로 활용 가능합니다. 
...
```
MySQL 을 사용하는 경우에는 `django.db.utils.OperationalError: (1071, 'Specified key was too long; max key length is 767 bytes')` 오류가 발생하여 다음과 같이 설정내용에 길이값을 명시적으로 선언 합니다.
{: .notice--info}

Celery 관리를 위한 **ORM 객체를** 생성 합니다.

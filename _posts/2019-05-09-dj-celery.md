...
odroid xu4 에서는 위 방식으로 설치한 결과 ` /usr/lib/python2.7/dist-packages/supervisor/options.py:297: UserWarning:` 오류가 발생하였습니다. **supervisor** 에서 다음 버젼부터는 Python 3 만 지원할 예정인 만큼 `sudo apt-get` 이 아닌 `$ sudo pip3 install` 을 사용하여 설치를 합니다. `supervisor` 는 redis 등에서 처리되는 프로세서 내용을 **daemon** 으로 위탁 관리하는 도구 입니다.

...
`supervisor` 는 redis 등에서 처리되는 프로세서 내용을 **daemon** 으로 위탁 관리하는 도구 입니다. `odroid xu4` 에서는 위 방식으로 설치한 결과 **/usr/lib/python2.7/dist-packages/supervisor/options.py:297: UserWarning:** 오류가 발생하였습니다. 자세한 설치내용은 앞에서 정리한 [Supervisord 모니터링](https://yongbeomkim.github.io/django/supervisord-ctl/) 를 활용합니다.

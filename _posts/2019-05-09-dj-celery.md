...
```python
In [1]: from stock.tasks import mul                                              In [2]: mul.delay(10,2)
...
**.delay(arg, kwarg=value)** 또는 **apply_asyncapply_async(eta=now + timedelta(seconds=10))** 메소드를 사용하면 비동기로 실행이 되고, 입력에 따른 Redis 반응 결과값을 관찰할 수 있습니다. [calling문서](http://docs.celeryproject.org/en/latest/userguide/calling.html)

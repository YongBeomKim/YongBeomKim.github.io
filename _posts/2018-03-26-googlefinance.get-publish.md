...
.
├── README.md
└── googlefinance.get      # import 모듈이름 
    └── __init__.py        # 함수명 (내부에 코드를 입력)
...
setup(
    ...
    keywords         = ['googlefinance', 'stock', 'price' , 'finance', 'googlefinance.get',],
...
`$ python setup.py bdist_wheel --universal`  패키지로 압축 
...
```python
# 왜 이름이 googlefinance로 되는지..
# 왜 2번 적어야 함수를 불러오는지가 모르겠당 ㅠㅠ...
from googlefinance.googlefinance import get_finance
get_finance()
```
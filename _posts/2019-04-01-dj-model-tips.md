...
**[StackOverFlow (Master Django p336, p339)](https://stackoverflow.com/questions/33350362/django-listview-form-to-filter-and-sort)** GenericView 를 잘 활용하면, 코드를 줄이고 내용들이 보다 명확하게 구현이 가능합니다. 하지만 개별 기능별로 url과 html 페이지를 따로 분리해야 하는데 이를 한 곳에 묶어서 정리하는 방식으로 구현하려면 부가적인 설계를 필요로 합니다.

이를 위해 사용가능한 객체로써 위의 2가지가 있고, 이들을 잘 활용하면 별도의 Jquery 의 Ajax 구조설계 없이도 다양한 기능을 덧붙일 수 있습니다.
```python

---
title : WAI-ARIA 의 개념과 role
last_modified_at: 2019-04-23T02:45:06-05:00
header:
...
## role
우선은 개별 tag 에 역활을 정의합니다. role 에 정의 가능한 내용들은 아래의 가이드 라인으로 정해져 있습니다. [Mozilla](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles) 

## aria- 
개별 태그에 위에서 설명한 역활(role) 을 부여한 뒤, 추가로 필요로 하는 **Properties(속성)** 과 **States(상태)** 를 정의 합니다. **속성** 은 처음 부여한 대로 활용 되지만, **상태** 는 ajax 등에 의해서 계속 변하는 값을 갖습니다.

<br/>
# 레진 WAI-ARIA 목록

다음에 서술할 가이드 라인은 [GitHub](https://github.com/lezhin/accessibility/blob/master/aria/README.md) 에서 가져온 내용입니다. [WAI-ARIA](https://www.w3.org/TR/wai-aria-1.1/)는 HTML의 접근성 문제를 보완하는 W3C 명세입니다. WAI-ARIA는 HTML 요소에 `role` 또는 `aria-*` 속성을 추가하여 콘텐츠의 '역할(roles), 상태(states), 속성(properties)' 정보를 보조기기에 제공합니다.

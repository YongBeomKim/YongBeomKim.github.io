...
간단하게 요약하면,chrome 의 headless 브라우저를 실행하는 **pyppeteer** 모듈이 **ARM** 에서 지원을 안했는데, 정식 **Chrome** 이 아닌 **chromium** 에서 **ARM** 환경을 지원하는 브라우저를 개발했는데, 모듈에는 이 내용이 반영되어 있지 않아서, 사용자가 별도로 브라우저를 설치하고 해당 내용을 반영하여 수정을 해야 한다는 내용입니다.

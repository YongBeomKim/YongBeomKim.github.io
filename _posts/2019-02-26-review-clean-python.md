...
이번 책은 이에반해 전체적인 PEP8 규칙의 흐름에 따라 구성되어 있어서 흐름을 따라가기에는 보다 수월합니다. 워의 책 보다는 기존에 출간된 책들 중 **객체지향 파이썬 프로그래밍** 이 책이 보다 더 가깝다고 보실 수 있습니다. 그런 측면에 있어서 두루 읽을만 한 책이라고 생각합니다. 
...
내용이 PEP8 의 전반적인 내용을 다루긴 하지만, 이들자체게 구체적인 기능구현을 하기 보다는, 구현된 내용을 보다 잘 다듬는 내용이기 때문에 어떠한 예시를 통해 설명을 하느냐가 중요하다고 봅니다. 이 책이 Packet 미국 원서를 번역한 내용이기 때문에 원본의 문제입니다만 전체에서 다룰 예시 코드 내용을 2~3 페이지로 설명한 뒤, 해당 내용을 개별 개념에 따라 어떻게 구현되고 차이가 있는지를 설명하면 더 잘 이해가 될텐데, 예시코드 자체가 이런 저런 상황을 넘나들고 있어서 아쉬웠습니다.

가장 아쉬운 부분은 색인이었습니다. **__init__, __call__** 과 같이 개념들이 많이 나열되는데, 이들을 책을 읽으면서 정리한 뒤 필요할 때 마다 찾을 수 있어야 활용하기 용이한데 반해 이 책의 색인 부분은 잘 정리된 개념들과 비교해 너무 부족합니다. 이 부분은 원서만이 아닌 출판사 입장에서 추가 및 개선이 가능할 수 있었을텐데 하는 아쉬움이 있습니다.

## 마무리
책의 내용을 읽고 진도를 나아가는데 있어서 적지않은 노력이 필요한 책 임을 부인하기 어렵습니다. 하지만 1회독을 정리하고 나면 이전보다 강력하고 완결성이 높은 모듈을 구연할 수 있도록 도와줄 수 있을 것 입ㄴ니다.

...
간단한 작업들은 if/ for 조건문을 과 List, Dict 객체만 잘 조합하면 구성이 가능하다. 하지만 대용량의 데이터 분석 작업을 진행할수록 **class, __init__, lambda** 등의 내용을 사용하면 보다 효과적인 진행이 가능함을 알 수 있다.

이를 정리하는 차원에서 데이터 전처리에  필요한 **itertools** 의 내용을 살펴 보고자 한다.
...
### itertools.count(시작 index, Step)
카운팅 인덱스 값을 사용자 임의의 값을 출력
...
전체 인덱스 값 중 **lambda 조건에 True** 인 값들을 출력합니다
...
attempts = [('dan', 87), ('erik', 95), ('jason', 79), 
    ('erik', 97), ('dan', 100)]

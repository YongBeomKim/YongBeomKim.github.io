---
title : 예비창업 Project
last_modified_at: 2019-06-02T10:45:06-05:00
header:
  overlay_image: /assets/images/book/Startup.png
categories:
  - business
tags: 
    - startup
    - business
---

**외부 데이터를 정기적** 으로 **수집 및 분석** 하고, 결과를 **아이템별/ 사용자별 맞춤형** 으로 제공하는 서비스를 제작하는 것이 이번 Project 의 목적 입니다.

1. **User Simple Interface**
2. **Fun and Beautiful Design**
3. **Machine Learning Analysis**

데이터를 수집하고 분석결과를 알려 주었다면, 자료들이 축적될 수록 **예측 데이터의 생성** 이 가능해 집니다.

식재료 관련 가격정보 및 비정형 데이터 수집 및 분석 결과제시

금융관련 정보수집 및 분석결과 제시, 빅데이터 데이터분석용 학습 Tutorial 사이트 만들며 분석내용 고도화 진행하기

<br/>
# 예산의 배정

1. 창업활동비 + 회계 기장비 550
2. 노트북, 데스크탑 500
3. 멘토비 800
4. 어플 외주용역 3,000

<br/>
# Scheduling

## 6월 : Service Template Design & Identity

서비스 Design 설계 및 세부적인 Identity 정의 하기

1. **계획서 내부의 Idea를 구체화** 하기 (속도전!!)
2. **Template Design** 및 **color** 정하기 (Try/ Error)
3. 법인화 작업 시작 및 **먹거리 창업센터** 지원

## 7월 : 법인의 설립 및 전시회 준비

**창업 후 3년** 지원 서비스로 창업시기를 늦출 수도 있지만, 주어진 10개월 내 속도를 내서 **본서비스, 금융서비스, 뉴스서비스** 를 런칭하고 다음단계 모색하기

1. 6, 7월 창업 활동비 **100 만원** 수집
2. 사업자 법인 세우기 절차 진행하기 (창업 3년내 지원 시기 계산)
3. 스마트시티 전시회 신청 및 작업 진행
4. 외주사와 작업지시서 상담 및 진행 시작하기

## 8월 : 데이터 관리 및 작업

**서울 빅데이터 캠퍼스** 와 **날씨마루** 등의 자료들로 분석 방법의 적용 및 다양한 파생변수 및 Dash Board 만들기

1. 특정 요일 합정동 사옥 출근 및 작업의 진행
2. 외주작업의 관리 및 구체적인 서비스 틀 갖추기

## 9월 : 스마트 도시 전시회 런칭 및 사용자 Test 진행

다양한 전문가와 멘토링을 통한 서비스 문제점 및 개선방향 찾기

## 11월 : 서비스 고도화 진행 및 중간결과의 발표




<figure class="align-center">
  <img src="http://www.datavirtualizationblog.com/wp-content/uploads/2015/11/3-phases-of-data-analysis-b.jpg">
  <figcaption></figcaption>
</figure>

<figure class="align-center">
  <img src="{{site.baseurl}}/assets/images/photo/hanyang.jpg">
  <figcaption></figcaption>
</figure>

|    추진내용        | 추진기간   |                                 세부내용 |
|:------------------:|:----------:|:----------------------------------------:|
| 데이터 수집 전처리 | ~ 2019.07  | 단일 메뉴 데이터 수집 및 식자재별 전처리 |


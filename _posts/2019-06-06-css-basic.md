...
**[반응형 웹 페이지](https://msdn.microsoft.com/ko-kr/library/bg124109)** 를 위한 개념들과 설정을 알아봅니다. 그리고 다양한 **아이콘 폰트** 와 함께, 애니메이션을 구현하는 **Animate.css** 와 **typewriterjs** 등 다양한 부가기능을 정리 합니다.
...
스타일을 입력하는 방법으로는 **1) Internal** 입력 방법과 **2) Inline** 입력 방법이 있습니다. **internal** 스타일이 관리에 용이하고, 바로 적용되는 내용을 확인하고 싶은 경우에는 **Inline** 으로 빠르게 작업을 합니다
...
입력방법으로 **Internal, Inline** 방법 2가지가 있고, **Internal** 로 작업시 동일한 값을 적용 후, 수정시 **변수명** 을 활용하면 유지보수 편의성이 높아 집니다. 아래의 방식으로 변수를 입력 합니다.
...
`:root{ }` 에서 변수명과 값을 정의한 뒤 `var()` CSS 함수를 활용하여 변수내 값을 호출 합니다.
...
객체 크기를 정의할 때 **px** 가 쉽지만 **반응형 웹** 의 단계별 크기에 대응하는 상태적인 객체크기를 다루는 단위 입니다.

**15vw :** 을 사용하는 경우 **윈도우 전체너비** 를 **100vm** 으로 기준 삼고서 상대크기를 자동으로 계산 합니다. **[w3school](https://www.w3schools.com/cssref/tryit.asp?filename=trycss_unit_em)** 등으로 실습하면 이해가 더 용이 합니다.
...
작업할 때에는 **모바일** 을 우선 정의한 뒤 **단계별로 스타일을 변경, 적용** 합니다.
...
flex 는 2개의 Tag를 필요로 합니다. **container** 와 **item** 으로 구분되고 이들에게 설정 가능한 속성들은 별개로 존재 합니다.

우선 객체간 유연한 배치정렬을 사용할 부모필드인 **container** 필드를 정의 합니다.
...
그리고 재배치를 할 자식인 **item** 에게 `flex` 범위를 구체적으로 입력 합니다.
...
보다 자세한 내용은 생활코딩 [YouTube](https://www.youtube.com/watch?v=BS9T0ZMqT58) 를 참고 합니다.
...
객체의 다양한 효과를 도와주는 **CSS** 모듈 입니다. [GitHub](https://github.com/daneden/animate.css) 소스코드를 확인해보면 **MIT 라이센스** 이고 지금까지도 개선이 충실하게 이루어 지고 있습니다.

## Example

https://www.cssscript.com/simple-typewriter-effect-pure-javascript-typewriterjs/

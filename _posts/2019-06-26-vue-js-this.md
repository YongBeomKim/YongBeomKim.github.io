...
**v-model** 은 **폼과 데이터를 바인딩** 하는 디렉티브 입니다. 아래의 예제와 같이 다양한 **Form** 타입에 적용 합니다.

단 주의할 점으로는 element 에 **초기에 정의된 값이 무시** 되고 **form, template** 의 데이터가 우선시 됩니다. 값이 무시될 뿐 **데이터 Type 은 유지되므로** 초기값으로 **데이터 Type** 를 정의 합니다.
...
select 를 사용하여 다양한 목록을 저장하는 **list** 객체를 활용할 수 있습니다. 이를 활용하면 데이터들이 누적되어 저장 됩니다.

...
## 1 일반현황
### 1-1 일반현황
- 일반 가정 및 중소식당을 위한 메뉴관리 및 식자재 정보 시스템
- 정보통신 김용범

### 1-2 창업 아이템 개요 (요약)
일반 가정과 중소식당 사용자를 대상으로, 공공데이터와 맛 지도 인터페이스를 활용한 식단 추천 및 식자재 정보 서비스

## 1 문제인식
### 1-1 창업 아이템의 개발동기
식단정보는 [**농촌진흥청 의 메뉴젠**](http://koreanfood.rda.go.kr/kfi/mgnmenudiaKcalnnutri/list) 과 [**청정원의 건강식단**](http://www.jungonenutri.com/user/diet/dietSearch.do?dietOrdYn=Y&pageNum=16&rdoGender=&dietCd=&gender=&age=&kcal=&rowCount=10#none) 에서와 같이(붙임2) 모범 메뉴목록과 식재료 목록들을 나열하고 있습니다. 실제 가격정보를 연결한 음식 메뉴별 식사재 원가 연동 시스템은 **삼성웰스토리, 현대그린푸드, 중고교 급식시스템** 과 같이 유통업체, 관리감독 기관에서 전문적인 영양사 분들의 대량주문을 위해 제작되어 있습니다. 때문에 일반 가정 및 중소식당에서 활용을 위한 솔루션은 부족합니다. 

### 1-2 창업 아이템의 목적 (필요성)
농산물은 가격 변동이 심한 만큼 과거에는 식자재 가격연동 시스템 구축이 어려웠지만 공공 데이터 개방과 함께, 다양한 농산물 온라인 유통이 활성화로 다영한 가격정보 접근이 쉬워 졌습니다. 
소비자 입장에서 맛 정보를 활용한 UI를 활용하는 등 보다 간단한 조작으로 접근성이 좋아진 서비스를 제공한다면, 중소식당 및 일반 가정에서도 불필요한 식자재 구매를 줄이고 다양한 재료의 가격을 비교함으로써 예산을 절약하는 등 다양한 혜택이 가능합니다

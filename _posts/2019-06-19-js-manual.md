...
**이벤트 핸들러** 방식으로 개별 내용들을 사용자가 지정하여 조작을 합니다.
...
이벤트 내용을 **인라인 방식으로** 추가 합니다. 별도의 자바스크립트가 아닌 **속성값인 이벤트로** 바로 추가 합니다. **방식이 간단한** 대신 **활용가능한 메서드가 한정적이고 유지보수가 어려운** 단점이 있습니다.
...
1. **onclick :** 마우스 클릭
1. **onmouseover :** hover 1단계
1. **onmouseout :** hover 2단계
1. **onkeydown :** 키보드입력
1. **onkeyup :** 키보드 눌렀다 놓을 때
1. **onfocus, onblur :** 요소 선택 및 해제시
1. **onsubmit :** form 이벤트 전송시
